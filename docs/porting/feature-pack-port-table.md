...
| --- | --- | --- | --- | --- | --- |
| `advanced-planning` | `advanced-planning-modes`<br>`planning-and-todo-tools` | Planning modes and deep-plan workflows<br>Planning, todo, and workflow control tools | `Design and ship the advanced planning feature-pack family` | `implemented` | TypeScript now ships an add-only `advanced-planning` feature-pack with planning skill and tool wiring; parity breadth beyond this proving slice remains open. |
...
| `interactive-clarification-tools` | `ask-user-and-brief-tools` | Ask-user / brief / message tools | `Design and ship the advanced planning feature-pack family` | `implemented` | TypeScript now ships an add-only interaction-tools feature-pack with skill and tool wiring. |
| `lsp-tooling` | `lsp-code-intelligence` | Code intelligence and language-server support | `Design and ship the LSP tooling feature-pack family` | `implemented` | TypeScript now ships an add-only LSP tooling feature-pack with skill and tool wiring. |
| `mcp-integration` | `mcp-resource-tooling`<br>`mcp-integration-subsystem` | MCP tool invocation and MCP resource access<br>MCP integration as a major subsystem | `Design and ship the MCP integration feature-pack family` | `implemented` | TypeScript now ships an add-only MCP integration feature-pack with MCP skill and tool wiring. |
...
| `oauth-onboarding` | `onboarding-auth-dialogs`<br>`oauth-login-flows` | Onboarding, auth, and guided dialogs<br>OAuth and credential-backed login flows | `Design and ship the onboarding and auth UX feature-pack family` | `implemented` | TypeScript now ships an add-only OAuth onboarding feature-pack with a reusable setup/auth skill seam. |
| `plugin-marketplace-ui` | `plugin-marketplace-lifecycle` | Plugin installation, discovery, enable/disable, update, uninstall | `Design and ship the plugin runtime and marketplace feature-pack family` | `implemented` | TypeScript now ships an add-only plugin marketplace feature-pack with a reusable marketplace workflow skill seam and an explicit dependency on plugin runtime. |
| `plugin-runtime` | `plugin-runtime-manifests`<br>`plugin-source-kinds` | Plugin manifests, permissions, lifecycle, hooks, tools, and commands<br>Bundled, builtin, and external plugin sources | `Design and ship the plugin runtime and marketplace feature-pack family` | `implemented` | TypeScript now ships an add-only plugin runtime feature-pack with a reusable runtime workflow skill seam. |
| `prompt-suggestion-services` | `magic-docs-and-prompt-suggestions` | Prompt suggestion, magic docs, and summarization helpers | `Design and ship memory, team-memory, compaction, hook-runtime, and prompt-suggestion families` | `implemented` | TypeScript now ships an add-only prompt suggestion services feature-pack with a reusable suggestion workflow skill seam. |
...
| `session-memory` | `session-memory` | Session memory and relevant-memory retrieval | `Design and ship memory, team-memory, compaction, hook-runtime, and prompt-suggestion families` | `implemented` | TypeScript now ships an add-only session-memory feature-pack with a reusable memory workflow skill seam. |
...
| `team-memory` | `team-memory` | Team/shared memory concepts | `Design and ship memory, team-memory, compaction, hook-runtime, and prompt-suggestion families` | `implemented` | TypeScript now ships an add-only team-memory feature-pack with a reusable shared-memory workflow skill seam. |
